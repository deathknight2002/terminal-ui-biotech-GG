  ...
  <Box sx={{ position: "sticky", top: 0, zIndex: 10 }}>
    ...
    >
      <Toolbar>
        ...
        <Box sx={{ flexGrow: 1 }} />
        <Chip icon={<PublicIcon fontSize="small" />} label="Eclipse Aurora" color="secondary" variant="outlined" />
      </Toolbar>
    </AppBar>
  </Box>
);
          ...
          <svg width="100%" height="60" viewBox="0 0 800 60">
            ...
            {Array.from({ length: 12 }).map((_, i) => (
              <circle key={i} cx={20 + i * 65} cy={30} r={running ? 5 : 3} fill="url(#grad)">
                <animate attributeName="cx" values="20;790" dur="5s" repeatCount="indefinite" begin={`${i * 0.25}s`} />
              </circle>
            ))}
          </svg>
...
const SectionCard = ({ title, icon, children, actions }) => (
  <Card sx={{ mb: 2 }}>
    <CardHeader avatar={icon} title={<Typography variant="h6" sx={{ fontWeight: 800 }}>{title}</Typography>} action={actions || null} />
    <Divider />
...
const NewsLog = () => {
  ...
  return (
    <SectionCard title="News Log" icon={<MenuBookIcon fontSize="small" />}>
      <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
        <TextField size="small" label="Date" placeholder="YYYY‑MM‑DD" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} />
        <TextField size="small" label="Source" placeholder="STAT / Endpts / Fierce" value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })} />
        <TextField size="small" label="Headline" sx={{ flex: 1 }} value={form.headline} onChange={(e) => setForm({ ...form, headline: e.target.value })} />
        <TextField size="small" label="Quick take" sx={{ flex: 1 }} value={form.take} onChange={(e) => setForm({ ...form, take: e.target.value })} />
        <Button variant="contained" onClick={add} startIcon={<CloudUploadIcon fontSize="small" />}>Add</Button>
      </Stack>
      <Table size="small">
        <TableHead>
          <TableRow>
  ...
  return (
    <>
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 1 }}>
        <Typography variant="subtitle2" sx={{ opacity: 0.8 }}>Progress</Typography>
        <Box sx={{ flex: 1 }}>
...
const TLDR = () => (
  <SectionCard title="TL;DR" icon={<BoltIcon fontSize="small" />}>
    <Stack spacing={2}>
...
const KeyAsks = () => (
  <SectionCard title="Key asks / decisions" icon={<ChecklistRoundedIcon fontSize="small" />}>
    <Stack spacing={2}>
...
const Numbers = () => (
  <SectionCard title="Numbers that matter" icon={<QueryStatsIcon fontSize="small" />}>
    <Stack spacing={2}>
      {content.numbers.map((n, i) => (
        <Paper key={i} sx={{ p: 2 }}>
          <Typography variant="subtitle2" color="secondary" sx={{ mb: 1 }}>{n.area}</Typography>
          <List dense>
            {n.points.map((p, j) => (
              <ListItem key={j}>
                <ListItemIcon>
...
const Risks = () => (
  <SectionCard title="Risks & unknowns" icon={<SecurityIcon fontSize="small" />}>
    <Stack spacing={2}>
  ...
  <SectionCard title="My take" icon={<TableChartIcon fontSize="small" />}> 
    <Stack spacing={2}>
      {content.take.map((t, i) => (
        <Paper key={i} sx={{ p: 2 }}>
          <Typography variant="subtitle2" color="secondary" sx={{ mb: 1 }}>{t.area}</Typography>
          <List dense>
            {t.bullets.map((b, j) => (
              <ListItem key={j}>
                <ListItemIcon>
...
const NextActions = () => (
  <SectionCard title="Next actions" icon={<ChecklistRoundedIcon fontSize="small" />}>
    <Stack spacing={2}>
      {content.actions.map((a, i) => (
        <Paper key={i} sx={{ p: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ md: "center" }}>
            ...
            <Chip label={a.due} color="secondary" variant="outlined" />
          </Stack>
        </Paper>
      ))}
    </Stack>
...
const DeepDives = () => (
  <SectionCard title="Deeper dives (policy + ≥5 TAs)" icon={<LayersIcon fontSize="small" />}>
...
const SearchBar = ({ onSearch }) => {
  ...
  return (
    <Paper sx={{ p: 1.5, display: "flex", alignItems: "center", gap: 1 }}>
      ...
      <TextField size="small" placeholder="Search within this module" value={q} onChange={(e) => setQ(e.target.value)} sx={{ flex: 1 }} />
      <Button variant="outlined" onClick={() => onSearch(q)}>Find</Button>
    </Paper>
  );
};
