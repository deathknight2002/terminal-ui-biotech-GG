  ...
  actions: [
    {
      ...
      detail: "15‑min AM scan of STAT/Endpts/Fierce; capture data readouts with quick takes",
    },
    {
      title: "PRV tracker",
      owner: "You",
  ...
  return (
    <>
      <Tooltip title="Copy to clipboard">
        <IconButton onClick={() => { navigator.clipboard.writeText(text); setOpen(true); }}>
          <ContentCopy size={18} />
        </IconButton>
      </Tooltip>
  ...
  return (
    <SectionCard title="News Log" icon={<BookOpen size={18} />}>
      <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
        ...
        <Button variant="contained" onClick={add} startIcon={<CloudUpload size={16} />}>Add</Button>
      </Stack>
      <Table size="small">
        <TableHead>
          <TableRow>
      ...
      {content.numbers.map((n, i) => (
        <Paper key={i} sx={{ p: 2 }}>
          ...
          <List dense>
            {n.points.map((p, j) => (
              <ListItem key={j}>
                <ListItemIcon><Layers size={16} /></ListItemIcon>
...
const SuggestedReplies = () => (
  <SectionCard title="Suggested reply lines to Laura" icon={<BookOpen size={18} />}
        ...
        <Container maxWidth="lg" sx={{ py: 3 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
            ...
            <Button variant="contained" startIcon={<Download size={16} />} onClick={() => {
              ...
              a.href = url; a.download = "aurora_module_export.json"; a.click(); URL.revokeObjectURL(url);
            }}>Export JSON</Button>
          </Box>
