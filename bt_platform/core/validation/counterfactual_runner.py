...
class CounterfactualConfig:
    ...
    vix_buckets: list[tuple[float, float]] = field(default_factory=lambda: [
        (0, 20), (20, 30), (30, 100)
    ])
...
class CounterfactualRunner:
    ...
    async def run_validation(self) -> dict[str, Any]:
        ...
        """
        logger.info(f"Starting counterfactual validation from {self.config.start_date} to {self.config.end_date}")
    ...
    async def _process_event(self, event: CounterfactualEvent) -> dict[str, Any]:
        ...
        edge = realized['pnl_bp_t5'] - cf_median_pnl if realized['pnl_bp_t5'] is not None else 0.0
    ...
    async def _get_realized_outcome(self, event: CounterfactualEvent) -> dict[str, Any]:
        ...
        stmt = select(RealizedOutcome).where(
            RealizedOutcome.event_id == event.event_id
        )
        result = self.db.execute(stmt)
    ...
    async def _generate_skip_counterfactual(self, event: CounterfactualEvent) -> dict[str, Any]:
    ...
    async def _find_alternative_catalysts(self, event: CounterfactualEvent) -> list[dict[str, Any]]:
        """
        ...
        if nearest:
            ...
            return [{
                ...
                'iv30_delta': (realized['iv30_post_t5'] - realized['iv30_pre']) if realized['iv30_post_t5'] and realized['iv30_pre'] else None,
    ...
    def _analyze_regime_performance(self, results: list[dict[str, Any]]) -> dict[str, RegimeMetrics]:
    ...
    def _detect_drift(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        scores = [r['score'] for r in sorted_results if r['score'] is not None and r['edge'] is not None]
        edges = [r['edge'] for r in sorted_results if r['score'] is not None and r['edge'] is not None]
    ...
    def _calculate_risk_metrics(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        - VaR and CVaR
        """
        pnls = [r['realized']['pnl_bp_t5'] for r in results
        ...
        cvar_95 = np.mean(pnls_array[pnls_array <= var_95]) if np.any(pnls_array <= var_95) else var_95
    ...
    def generate_report(self, results: dict[str, Any], output_path: str) -> None:
        ...
        with open(output_path, 'w') as f:
            ...
            f.write(f"- **Drift Detected:** {'Yes' if drift.get('drift_detected') else 'No'}\n")
...
def create_counterfactual_id(event_id: str, cf_type: str, alt_ticker: str | None = None) -> str:
