...
async def get_moa_data(target: str, db: Session = Depends(get_db)):
    """
    ...
    target_data = {
        ...
        "TL1A": {
            ...
            "genetic_evidence": {
                "source": "Open Targets",
                "score": 0.72,
                "associations": [{"disease": "IBD (combined)", "score": 0.74}],
            },
            "bench_potency": {
                "source": "ChEMBL",
                "ic50": "15 nM",
...
async def get_company_scorecard(company: str, db: Session = Depends(get_db)):
    """
